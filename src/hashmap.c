...
HASHMAP_STATUS
HashMap_Initialize(PHASHMAP pHashMap, size_t cInitialCapacity,
                   HASHMAP_FREE_ROUTINE pFreeFn, void *pFreeFnUserData) {

    pHashMap->cCapacity = cInitialCapacity;
    pHashMap->cSize = 0;
    pHashMap->pFreeFn = pFreeFn;
    pHashMap->pFreeFnUserData = pFreeFnUserData;
    pHashMap->pElements = malloc(sizeof(HASHMAP_ELEMENT) * cInitialCapacity);

    memset(pHashMap->pElements, 0, sizeof(HASHMAP_ELEMENT) * cInitialCapacity);

    return (pHashMap->pElements != NULL) ? HASHMAP_STATUS_OK
...
HashMap_Put(PHASHMAP pHashMap, char const *pszKey, void *pValue) {
    ...
    if (!pElement->pData) {
        ...
        for (; pCurrent && (bMatch = strcmp(pCurrent->pszKey, pszKey) != 0);
             pCurrent = pCurrent->pNext)
            ;

        if (!bMatch) {
            if (pHashMap->pFreeFn) {
                pHashMap->pFreeFn(pCurrent->pData, pHashMap->pFreeFnUserData);
            }
...
void HashMap_DataMapFn(PHASHMAP pHashMap, HASHMAP_MAP_ROUTINE MapFn,
    ...
    for (size_t i = 0; i < pHashMap->cCapacity; i++) {
        ...
        if (pElement->pData != NULL) {
            for (PHASHMAP_ELEMENT pCurrent = pElement; pCurrent;
                 pCurrent = pCurrent->pNext) {
...
HASHMAP_STATUS
HashMap_Get(PHASHMAP pHashMap, char const *pszKey, void **pOutData) {
    size_t hash = __HashMap_Hash(pszKey);
    PHASHMAP_ELEMENT pElement =
        pHashMap->pElements + (hash % pHashMap->cCapacity);
    PHASHMAP_ELEMENT pCurrent = pElement;
    ...
    for (; pCurrent && (strcmp(pCurrent->pszKey, pszKey) != 0);
         pCurrent = pCurrent->pNext)
        ;

    if (pCurrent) {
        *pOutData = pCurrent->pData;
        return HASHMAP_STATUS_OK;
    }
